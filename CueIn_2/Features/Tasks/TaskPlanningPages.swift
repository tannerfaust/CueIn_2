    ...
    var body: some View {
        VStack(alignment: .leading, spacing: CueInSpacing.md) {
            HStack(spacing: CueInSpacing.md) {
                ZStack {
                    ...
                    Image(systemName: field.resolvedIconSystemName)
                ...
                HStack(spacing: CueInSpacing.sm) {
                    ForEach(projects.prefix(3)) { project in
                        Text(project.name)
                            ...
                            .padding(.vertical, 5)
                            .background(store.color(for: project).opacity(0.12))
                            .clipShape(Capsule())
                    }
                    Spacer(minLength: 0)
                }
        ...
        .overlay {
            RoundedRectangle(cornerRadius: CueInSpacing.cardRadius, style: .continuous)
                .strokeBorder(field.color.opacity(0.18), lineWidth: 0.6)
        }
    }
}
    ...
    var body: some View {
        ScrollView(.vertical, showsIndicators: false) {
            VStack(alignment: .leading, spacing: CueInSpacing.xl) {
                ...
                ForEach(store.fields) { field in
                    let projects = store.projects(in: field.id)
                    ProjectGroupSection(
                        field: field,
                        projects: projects,
                        store: store,
                        onCreateProject: { onCreateProject(field.id) },
